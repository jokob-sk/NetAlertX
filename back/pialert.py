...
def fixPermissions():
    ...
    for com in chmodCommands:
        ...
        try:
            ...
            mylog('none', [
                  "[Setup] Fix Failed. Execute this command manually inside of the container: ", ' '.join(com)])
            mylog('none', [e.output])
...
def initialiseFile(pathToCheck, defaultFile):
    ...
    if str(os.access(pathToCheck, os.R_OK)) == "False":
        mylog('none', ["[Setup] (" + pathToCheck +
              ") file is not readable or missing. Trying to copy over the default one."])
        try:
            # try runnning a subprocess
            p = subprocess.Popen(["cp", defaultFile, pathToCheck],
                                 stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            stdout, stderr = p.communicate()

            if str(os.access(pathToCheck, os.R_OK)) == "False":
                mylog('none', ["[Setup] Error copying ("+defaultFile+") to ("+pathToCheck +
            ...
            # An error occured, handle it
            mylog('none', ["[Setup] Error copying ("+defaultFile +
                  "). Make sure the app has Read & Write access to " + pathToCheck])
            mylog('none', [e.output])
...
def ccd(key, default, config, name, inputtype, options, group, events=[], desc="", regex=""):
    ...
    mySettingsSQLsafe.append(
        (key, name, desc, inputtype, options, regex, str(result), group, str(events)))
    mySettings.append((key, name, desc, inputtype, options,
                      regex, result, group, str(events)))
...
def importConfigs():
    ...
    # General
    ENABLE_ARPSCAN = ccd('ENABLE_ARPSCAN', True, c_d,
                         'Enable arpscan', 'boolean', '', 'General', ['run'])
    SCAN_SUBNETS = ccd('SCAN_SUBNETS', ['192.168.1.0/24 --interface=eth1',
                       '192.168.1.0/24 --interface=eth0'], c_d, 'Subnets to scan', 'subnets', '', 'General')
    LOG_LEVEL = ccd('LOG_LEVEL', 'verbose', c_d, 'Log verboseness',
                    'selecttext', "['none', 'minimal', 'verbose', 'debug']", 'General')
    TIMEZONE = ccd('TIMEZONE', 'Europe/Berlin', c_d,
                   'Time zone', 'text', '', 'General')
    ENABLE_PLUGINS = ccd('ENABLE_PLUGINS', True, c_d,
                         'Enable plugins', 'boolean', '', 'General')
    PIALERT_WEB_PROTECTION = ccd(
        'PIALERT_WEB_PROTECTION', False, c_d, 'Enable logon', 'boolean', '', 'General')
    PIALERT_WEB_PASSWORD = ccd('PIALERT_WEB_PASSWORD', '8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92',
                               c_d, 'Logon password', 'readonly', '', 'General')
    INCLUDED_SECTIONS = ccd('INCLUDED_SECTIONS', ['internet', 'new_devices', 'down_devices', 'events', 'ports'], c_d,
                            'Notify on', 'multiselect', "['internet', 'new_devices', 'down_devices', 'events', 'ports', 'plugins']", 'General')
    SCAN_CYCLE_MINUTES = ccd('SCAN_CYCLE_MINUTES', 5, c_d,
                             'Scan cycle delay (m)', 'integer', '', 'General')
    DAYS_TO_KEEP_EVENTS = ccd(
        'DAYS_TO_KEEP_EVENTS', 90, c_d, 'Delete events days', 'integer', '', 'General')
    REPORT_DASHBOARD_URL = ccd(
        'REPORT_DASHBOARD_URL', 'http://pi.alert/', c_d, 'PiAlert URL', 'text', '', 'General')
    DIG_GET_IP_ARG = ccd('DIG_GET_IP_ARG', '-4 myip.opendns.com @resolver1.opendns.com',
                         c_d, 'DIG arguments', 'text', '', 'General')
    UI_LANG = ccd('UI_LANG', 'English', c_d, 'Language Interface',
                  'selecttext', "['English', 'German', 'Spanish']", 'General')

    # Email
    REPORT_MAIL = ccd('REPORT_MAIL', False, c_d, 'Enable email',
                      'boolean', '', 'Email', ['test'])
    SMTP_SERVER = ccd('SMTP_SERVER', '', c_d,
                      'SMTP server URL', 'text', '', 'Email')
    SMTP_PORT = ccd('SMTP_PORT', 587, c_d, 'SMTP port', 'integer', '', 'Email')
    REPORT_TO = ccd('REPORT_TO', 'user@gmail.com', c_d,
                    'Email to', 'text', '', 'Email')
    REPORT_FROM = ccd('REPORT_FROM', 'Pi.Alert <user@gmail.com>',
                      c_d, 'Email Subject', 'text', '', 'Email')
    SMTP_SKIP_LOGIN = ccd('SMTP_SKIP_LOGIN', False, c_d,
                          'SMTP skip login', 'boolean', '', 'Email')
    SMTP_USER = ccd('SMTP_USER', '', c_d, 'SMTP user', 'text', '', 'Email')
    SMTP_PASS = ccd('SMTP_PASS', '', c_d, 'SMTP password',
                    'password', '', 'Email')
    SMTP_SKIP_TLS = ccd('SMTP_SKIP_TLS', False, c_d,
                        'SMTP skip TLS', 'boolean', '', 'Email')
    SMTP_FORCE_SSL = ccd('SMTP_FORCE_SSL', False, c_d,
                         'Force SSL', 'boolean', '', 'Email')

    # Webhooks
    REPORT_WEBHOOK = ccd('REPORT_WEBHOOK', False, c_d,
                         'Enable Webhooks', 'boolean', '', 'Webhooks', ['test'])
    WEBHOOK_URL = ccd('WEBHOOK_URL', '', c_d,
                      'Target URL', 'text', '', 'Webhooks')
    WEBHOOK_PAYLOAD = ccd('WEBHOOK_PAYLOAD', 'json', c_d, 'Payload type',
                          'selecttext', "['json', 'html', 'text']", 'Webhooks')
    WEBHOOK_REQUEST_METHOD = ccd('WEBHOOK_REQUEST_METHOD', 'GET', c_d,
                                 'Req type', 'selecttext', "['GET', 'POST', 'PUT']", 'Webhooks')

    # Apprise
    REPORT_APPRISE = ccd('REPORT_APPRISE', False, c_d,
                         'Enable Apprise', 'boolean', '', 'Apprise', ['test'])
    APPRISE_HOST = ccd('APPRISE_HOST', '', c_d,
                       'Apprise host URL', 'text', '', 'Apprise')
    APPRISE_URL = ccd('APPRISE_URL', '', c_d,
                      'Apprise notification URL', 'text', '', 'Apprise')
    APPRISE_PAYLOAD = ccd('APPRISE_PAYLOAD', 'html', c_d,
                          'Payload type', 'selecttext', "['html', 'text']", 'Apprise')

    # NTFY
    REPORT_NTFY = ccd('REPORT_NTFY', False, c_d, 'Enable NTFY',
                      'boolean', '', 'NTFY', ['test'])
    NTFY_HOST = ccd('NTFY_HOST', 'https://ntfy.sh', c_d,
                    'NTFY host URL', 'text', '', 'NTFY')
    NTFY_TOPIC = ccd('NTFY_TOPIC', '', c_d, 'NTFY topic', 'text', '', 'NTFY')
    NTFY_USER = ccd('NTFY_USER', '', c_d, 'NTFY user', 'text', '', 'NTFY')
    NTFY_PASSWORD = ccd('NTFY_PASSWORD', '', c_d,
                        'NTFY password', 'password', '', 'NTFY')
    ...
    # PUSHSAFER
    REPORT_PUSHSAFER = ccd('REPORT_PUSHSAFER', False, c_d,
                           'Enable PUSHSAFER', 'boolean', '', 'PUSHSAFER', ['test'])
    PUSHSAFER_TOKEN = ccd('PUSHSAFER_TOKEN', 'ApiKey', c_d,
                          'PUSHSAFER token', 'text', '', 'PUSHSAFER')

    # MQTT
    REPORT_MQTT = ccd('REPORT_MQTT', False, c_d,
                      'Enable MQTT', 'boolean', '', 'MQTT')
    MQTT_BROKER = ccd('MQTT_BROKER', '', c_d,
                      'MQTT broker', 'text', '', 'MQTT')
    MQTT_PORT = ccd('MQTT_PORT', 1883, c_d,
                    'MQTT broker port', 'integer', '', 'MQTT')
    MQTT_USER = ccd('MQTT_USER', '', c_d, 'MQTT user', 'text', '', 'MQTT')
    MQTT_PASSWORD = ccd('MQTT_PASSWORD', '', c_d,
                        'MQTT password', 'password', '', 'MQTT')
    MQTT_QOS = ccd('MQTT_QOS', 0, c_d, 'MQTT Quality of Service',
                   'selectinteger', "['0', '1', '2']", 'MQTT')
    MQTT_DELAY_SEC = ccd('MQTT_DELAY_SEC', 2, c_d, 'MQTT delay',
                         'selectinteger', "['2', '3', '4', '5']", 'MQTT')

    # DynDNS
    DDNS_ACTIVE = ccd('DDNS_ACTIVE', False, c_d,
                      'Enable DynDNS', 'boolean', '', 'DynDNS')
    DDNS_DOMAIN = ccd('DDNS_DOMAIN', 'your_domain.freeddns.org',
                      c_d, 'DynDNS domain URL', 'text', '', 'DynDNS')
    DDNS_USER = ccd('DDNS_USER', 'dynu_user', c_d,
                    'DynDNS user', 'text', '', 'DynDNS')
    DDNS_PASSWORD = ccd('DDNS_PASSWORD', 'A0000000B0000000C0000000D0000000',
                        c_d, 'DynDNS password', 'password', '', 'DynDNS')
    DDNS_UPDATE_URL = ccd('DDNS_UPDATE_URL', 'https://api.dynu.com/nic/update?',
                          c_d, 'DynDNS update URL', 'text', '', 'DynDNS')

    # PiHole
    PIHOLE_ACTIVE = ccd('PIHOLE_ACTIVE',  False, c_d,
                        'Enable PiHole mapping', 'boolean', '', 'PiHole')
    DHCP_ACTIVE = ccd('DHCP_ACTIVE', False, c_d,
                      'Enable PiHole DHCP', 'boolean', '', 'PiHole')

    # PHOLUS
    PHOLUS_ACTIVE = ccd('PHOLUS_ACTIVE', False, c_d,
                        'Enable Pholus scans', 'boolean', '', 'Pholus')
    PHOLUS_TIMEOUT = ccd('PHOLUS_TIMEOUT', 20, c_d,
                         'Pholus timeout', 'integer', '', 'Pholus')
    PHOLUS_FORCE = ccd('PHOLUS_FORCE', False, c_d,
                       'Pholus force check', 'boolean', '', 'Pholus')
    PHOLUS_RUN = ccd('PHOLUS_RUN', 'once', c_d, 'Pholus enable schedule',
                     'selecttext', "['none', 'once', 'schedule']", 'Pholus')
    PHOLUS_RUN_TIMEOUT = ccd('PHOLUS_RUN_TIMEOUT', 600, c_d,
                             'Pholus timeout schedule', 'integer', '', 'Pholus')
    PHOLUS_RUN_SCHD = ccd('PHOLUS_RUN_SCHD', '0 4 * * *',
                          c_d, 'Pholus schedule', 'text', '', 'Pholus')
    PHOLUS_DAYS_DATA = ccd('PHOLUS_DAYS_DATA', 0, c_d,
                           'Pholus keep days', 'integer', '', 'Pholus')

    # Nmap
    NMAP_ACTIVE = ccd('NMAP_ACTIVE', True, c_d,
                      'Enable Nmap scans', 'boolean', '', 'Nmap')
    NMAP_TIMEOUT = ccd('NMAP_TIMEOUT', 150, c_d,
                       'Nmap timeout', 'integer', '', 'Nmap')
    NMAP_RUN = ccd('NMAP_RUN', 'none', c_d, 'Nmap enable schedule',
                   'selecttext', "['none', 'once', 'schedule']", 'Nmap')
    NMAP_RUN_SCHD = ccd('NMAP_RUN_SCHD', '0 2 * * *', c_d,
                        'Nmap schedule', 'text', '', 'Nmap')
    NMAP_ARGS = ccd('NMAP_ARGS', '-p -10000', c_d,
                    'Nmap custom arguments', 'text', '', 'Nmap')

    # API
    API_CUSTOM_SQL = ccd('API_CUSTOM_SQL', 'SELECT * FROM Devices WHERE dev_PresentLastScan = 0',
                         c_d, 'Custom endpoint', 'text', '', 'API')
    ...
    pholusSchedule = Cron(PHOLUS_RUN_SCHD).schedule(
        start_date=datetime.datetime.now(tz))
    mySchedules.append(schedule_class(
        "pholus", pholusSchedule, pholusSchedule.next(), False))

    # init nmap schedule
    nmapSchedule = Cron(NMAP_RUN_SCHD).schedule(
        start_date=datetime.datetime.now(tz))
    mySchedules.append(schedule_class(
        "nmap", nmapSchedule, nmapSchedule.next(), False))
    ...
    if ENABLE_PLUGINS:
        ...
        mylog('none', [
              '[', timeNow(), '] Plugins: Number of dynamically loaded plugins: ', len(plugins)])
        ...
        for plugin in plugins:
            ...
            for set in plugin["settings"]:
                ...
                v = ccd(key, set["default_value"], c_d, set["name"][0]
                        ["string"], set["type"], str(set["options"]), pref)
                ...
                if setFunction == 'RUN_SCHD':
                    newSchedule = Cron(v).schedule(
                        start_date=datetime.datetime.now(tz))
                    mySchedules.append(schedule_class(
                        pref, newSchedule, newSchedule.next(), False))
...
# update vendors 24h after first run and then once a week
last_update_vendors = time_started - datetime.timedelta(days=6)
    ...
    while True:
        ...
        if last_run + datetime.timedelta(minutes=1) < time_started:
            ...
            if PHOLUS_RUN == "schedule" or PHOLUS_RUN == "once":
                ...
                pholusSchedule = [
                    sch for sch in mySchedules if sch.service == "pholus"][0]
                run = False
                ...
                if run:
                    pholusSchedule.last_run = datetime.datetime.now(
            ...
            if NMAP_RUN == "schedule" or NMAP_RUN == "once":
                ...
                nmapSchedule = [
                    sch for sch in mySchedules if sch.service == "nmap"][0]
                run = False
                ...
                if run:
                    nmapSchedule.last_run = datetime.datetime.now(
                        tz).replace(microsecond=0)
...
def cleanup_database():
    ...
    # Cleanup Events
    mylog('verbose', ['    Events: Delete all older than ' +
          str(DAYS_TO_KEEP_EVENTS)+' days'])
    sql.execute("DELETE FROM Events WHERE eve_DateTime <= date('now', '-" +
                str(DAYS_TO_KEEP_EVENTS)+" day')")

    # Cleanup Plugin Events History
    mylog('verbose', [
          '    Plugin Events History: Delete all older than '+str(DAYS_TO_KEEP_EVENTS)+' days'])
    sql.execute("DELETE FROM Plugins_History WHERE DateTimeChanged <= date('now', '-" +
                str(DAYS_TO_KEEP_EVENTS)+" day')")

    # Cleanup Pholus_Scan
    if PHOLUS_DAYS_DATA != 0:
        mylog('verbose', [
              '    Pholus_Scan: Delete all older than ' + str(PHOLUS_DAYS_DATA) + ' days'])
        # improvement possibility: keep at least N per mac
        sql.execute("DELETE FROM Pholus_Scan WHERE Time <= date('now', '-" +
                    str(PHOLUS_DAYS_DATA) + " day')")
...
def update_devices_MAC_vendors(pArg=''):
    ...
    try:
        ...
        # An error occured, handle it
        mylog(
            'none', ['    FAILED: Updating vendors DB, set LOG_LEVEL=debug for more info'])
        mylog('none', [e.output])
...
def save_scanned_devices(p_arpscan_devices, p_cycle_interval):
    ...
    local_mac = subprocess.Popen(local_mac_cmd, shell=True, stdout=subprocess.PIPE,
                                 stderr=subprocess.STDOUT).communicate()[0].decode().strip()
    ...
    local_ip = subprocess.Popen(local_ip_cmd, shell=True, stdout=subprocess.PIPE,
                                stderr=subprocess.STDOUT).communicate()[0].decode().strip()
    ...
    sql.execute(
        "SELECT COUNT(*) FROM CurrentScan WHERE cur_ScanCycle = ? AND cur_MAC = ? ", (cycle, local_mac))
    if sql.fetchone()[0] == 0:
        sql.execute("INSERT INTO CurrentScan (cur_ScanCycle, cur_MAC, cur_IP, cur_Vendor, cur_ScanMethod) " +
...
def print_scan_stats():
    ...
    sql.execute("""SELECT COUNT(*) FROM CurrentScan
                ...
                (cycle,))
    mylog('verbose', ['        Pi-hole detected...: +' +
          str(sql.fetchone()[0])])
    ...
    mylog('verbose', ['    Devices in this cycle..: ' +
          str(sql.fetchone()[0])])
...
def insertOnlineHistory():
    ...
    sql.execute(
        """SELECT * FROM CurrentScan WHERE cur_ScanCycle = ? """, (cycle,))
    History_Online = sql.fetchall()
    History_Online_Devices = len(History_Online)
    History_Offline_Devices = History_All_Devices - \
        History_Archived_Devices - History_Online_Devices
...
def update_devices_names():
    ...
    sql.execute(
        "SELECT * FROM Devices WHERE dev_Name IN ('(unknown)','', '(name not found)') AND dev_LastIP <> '-'")
    ...
    mylog('verbose', [
          "          Pholus entries from prev scans: ", len(pholusResults)])

    for device in unknownDevices:
        newName = -1

        # Resolve device name with DiG
        newName = resolve_device_name_dig(
            device['dev_MAC'], device['dev_LastIP'])
        ...
        # Resolve with Pholus
        if newName == -1:
            newName = resolve_device_name_pholus(
                device['dev_MAC'], device['dev_LastIP'], pholusResults)
            # count
            if newName != -1:
                foundPholus += 1
    ...
    # Print log
    mylog('verbose', ["        Names Found (DiG/Pholus): ",
          len(recordsToUpdate), " (", foundDig, "/", foundPholus, ")"])
    mylog('verbose', ["        Names Not Found         : ",
          len(recordsNotFound)])

    # update not found devices with (name not found)
    sql.executemany(
        "UPDATE Devices SET dev_Name = ? WHERE dev_MAC = ? ", recordsNotFound)
    # update names of devices which we were bale to resolve
    sql.executemany(
        "UPDATE Devices SET dev_Name = ? WHERE dev_MAC = ? ", recordsToUpdate)
    commitDB()
...
def performNmapScan(devicesToScan):
    ...
    if len(devicesToScan) > 0:
        ...
        mylog('verbose', ['[', timeNow(), '] Scan: Nmap for max ', str(
            timeoutSec), 's (' + str(round(int(timeoutSec) / 60, 1)) + 'min) per device'])

        mylog('verbose', ["        Estimated max delay: ", (devTotal * int(timeoutSec)),
              's ', '(', round((devTotal * int(timeoutSec))/60, 1), 'min)'])
        ...
        for device in devicesToScan:
            ...
            try:
                ...
                output = subprocess.check_output(
                    nmapArgs, universal_newlines=True,  stderr=subprocess.STDOUT, timeout=(timeoutSec + 30))
            except subprocess.CalledProcessError as e:
                # An error occured, handle it
                mylog('none', [e.output])
                mylog(
                    'none', ["        Error - Nmap Scan - check logs", progress])
            except subprocess.TimeoutExpired as timeErr:
                mylog('verbose', [
                      '        Nmap TIMEOUT - the process forcefully terminated as timeout reached for ', device["dev_LastIP"], progress])

            if output == "":  # check if the subprocess failed
                mylog('info', ['[', timeNow(), '] Scan: Nmap FAIL for ',
                      device["dev_LastIP"], progress, ' check logs for details'])
            else:
                mylog('verbose', [
                      '[', timeNow(), '] Scan: Nmap SUCCESS for ', device["dev_LastIP"], progress])
            ...
            for line in newLines:
                if 'Starting Nmap' in line:
                    ...
                    newEntriesTmp.append(nmap_entry(device["dev_MAC"], timeNow(), line.split()[
                                         0], line.split()[1], line.split()[2], device["dev_Name"]))
            ...
            mylog('verbose', [
                  '[', timeNow(), '] Scan: Ports found by NMAP: ', len(newEntriesTmp)])
            ...
            if len(newEntriesTmp) > 0:
                ...
                for row in rows:
                    ...
                    if row["MAC"] == device["dev_MAC"]:
                        oldEntries.append(nmap_entry(
                            row["MAC"], row["Time"], row["Port"], row["State"], row["Service"], device["dev_Name"], row["Extra"], row["Index"]))
                ...
                mylog('verbose', [
                      '[', timeNow(), '] Scan: Nmap newly discovered or changed ports: ', len(newEntries)])
                ...
                if len(newEntries) > 0:
                    ...
                    for newEntry in newEntries:
                        ...
                        columnNames = ["Name", "MAC", "Port",
                                       "State", "Service", "Extra", "NewOrOld"]

                        # Old entry found
                        if foundEntry is not None:
                            # Build params for sql query
                            params.append((newEntry.mac, newEntry.time, newEntry.port,
                                          newEntry.state, newEntry.service, oldEntry.extra))
                        ...
                        # New entry - no matching Old entry found
                            ...
                            # Build params for sql query
                            params.append(
                                (newEntry.mac, newEntry.time, newEntry.port, newEntry.state, newEntry.service, ''))
                            # Build JSON for API and notifications
                            changedPortsTmp.append({
                                "Name": "New device",
                                "MAC": newEntry.mac,
                    ...
                    changedPorts_json_struc = json_struc(
                        {"data": changedPortsTmp}, columnNames)

                    #  Delete old entries if available
                    if len(indexesToDelete) > 0:
                        sql.execute(
                            "DELETE FROM Nmap_Scan where \"Index\" in (" + indexesToDelete[:-1] + ")")
                        commitDB()

                    # Insert new values into the DB
                    sql.executemany(
                        """INSERT INTO Nmap_Scan ("MAC", "Time", "Port", "State", "Service", "Extra") VALUES (?, ?, ?, ?, ?, ?)""", params)
                    commitDB()
...
def performPholusScan(timeoutSec):
    ...
    for subnet in userSubnets:
        ...
        if len(temp) != 2:
            mylog('none', [
                  "        Skip scan (need subnet in format '192.168.1.0/24 --inteface=eth0'), got: ", subnet])
            return
        ...
        mylog('info', ['[', timeNow(), '] Scan: Pholus for ', str(
            timeoutSec), 's (' + str(round(int(timeoutSec) / 60, 1)) + 'min)'])
        mylog('verbose', ["        Pholus scan on [interface] ",
              interface, " [mask] ", mask])
        ...
        pholus_args = ['python3', '/home/pi/pialert/pholus/pholus3.py',
                       interface, "-rdns_scanning", mask, "-stimeout", adjustedTimeout]
        ...
        try:
            # try runnning a subprocess with a forced (timeout + 30 seconds)  in case the subprocess hangs
            output = subprocess.check_output(
                pholus_args, universal_newlines=True,  stderr=subprocess.STDOUT, timeout=(timeoutSec + 30))
        except subprocess.CalledProcessError as e:
            # An error occured, handle it
            mylog('none', [e.output])
            ...
            mylog('none', [
                  '        Pholus TIMEOUT - the process forcefully terminated as timeout reached'])
        ...
        for line in newLines:
            columns = line.split("|")
            if len(columns) == 4:
                params.append((interface + " " + mask, timeNow(), columns[0].replace(
                    " ", ""), columns[1].replace(" ", ""), columns[2].replace(" ", ""), columns[3], ''))

        if len(params) > 0:
            sql.executemany(
                """INSERT INTO Pholus_Scan ("Info", "Time", "MAC", "IP_v4_or_v6", "Record_Type", "Value", "Extra") VALUES (?, ?, ?, ?, ?, ?, ?)""", params)
            commitDB()
...
def resolve_device_name_dig(pMAC, pIP):
    ...
    try:
        ...
        try:
            # try runnning a subprocess
            newName = subprocess.check_output(
                dig_args, universal_newlines=True)
        except subprocess.CalledProcessError as e:
            # An error occured, handle it
            mylog('none', [e.output])
...
def send_notifications():
    ...
    if isNewVersion():
        template_file = open(
            pialertPath + '/back/report_template_new_version.html', 'r')
    ...
    if 'internet' in INCLUDED_SECTIONS:
        ...
        mail_text = mail_text.replace(
            '<SECTION_INTERNET>', notiStruc.text + '\n')
    ...
    if 'new_devices' in INCLUDED_SECTIONS:
        ...
        mail_text = mail_text.replace(
            '<SECTION_NEW_DEVICES>', notiStruc.text + '\n')
    ...
    if 'down_devices' in INCLUDED_SECTIONS:
        ...
        mail_text = mail_text.replace(
            '<SECTION_DEVICES_DOWN>', notiStruc.text + '\n')
    ...
    if 'ports' in INCLUDED_SECTIONS:
        ...
        notiStruc = construct_notifications(
            "", "Ports", True, changedPorts_json_struc)
    ...
    if json_internet != [] or json_new_devices != [] or json_down_devices != [] or json_events != [] or json_ports != [] or debug_force_notification or plugins_report:
        ...
        if REPORT_PUSHOVER and check_config('pushover'):
            updateState("Send: PUSHOVER")
            mylog('info', ['      Sending report by PUSHOVER'])
            send_pushover(mail_text)
        else:
            mylog('verbose', ['      Skip PUSHOVER'])
...
def construct_notifications(sqlQuery, tableTitle, skipText=False, suppliedJsonStruct=None):
    ...
    table_attributes = {"style": "border-collapse: collapse; font-size: 12px; color:#70707",
                        "width": "100%", "cellspacing": 0, "cellpadding": "3px", "bordercolor": "#C0C0C0", "border": "1"}
    ...
    if len(jsn["data"]) > 0:
        ...
        html = convert(jsn, build_direction=build_direction,
        ...
        if skipText == False:
            ...
            for device in jsn["data"]:
                for header in headers:
                    ...
                    if len(header) < 4:
                        padding = "\t"
                    text += text_line.format(header +
                                             ': ' + padding, device[header])
                text += '\n'
...
def check_config(service):
    ...
    if service == 'email':
        if SMTP_PASS == '' or SMTP_SERVER == '' or SMTP_USER == '' or REPORT_FROM == '' or REPORT_TO == '':
            mylog('none', [
                  '    Error: Email service not set up correctly. Check your pialert.conf SMTP_*, REPORT_FROM and REPORT_TO variables.'])
            return False
        else:
            return True
    ...
    if service == 'apprise':
        if APPRISE_URL == '' or APPRISE_HOST == '':
            mylog('none', [
                  '    Error: Apprise service not set up correctly. Check your pialert.conf APPRISE_* variables.'])
            return False
        else:
            return True
    ...
    if service == 'webhook':
        if WEBHOOK_URL == '':
            mylog('none', [
                  '    Error: Webhook service not set up correctly. Check your pialert.conf WEBHOOK_* variables.'])
            return False
        else:
            return True
    ...
    if service == 'ntfy':
        if NTFY_HOST == '' or NTFY_TOPIC == '':
            mylog('none', [
                  '    Error: NTFY service not set up correctly. Check your pialert.conf NTFY_* variables.'])
            return False
        else:
            return True
    if service == 'pushover':
    ...
    if service == 'pushsafer':
        if PUSHSAFER_TOKEN == 'ApiKey':
            mylog('none', [
                  '    Error: Pushsafer service not set up correctly. Check your pialert.conf PUSHSAFER_TOKEN variable.'])
            return False
        else:
            return True
    ...
    if service == 'mqtt':
        if MQTT_BROKER == '' or MQTT_PORT == '' or MQTT_USER == '' or MQTT_PASSWORD == '':
            mylog('none', [
                  '    Error: MQTT service not set up correctly. Check your pialert.conf MQTT_* variables.'])
            return False
        else:
            return True
...
def generate_mac_links(html, deviceUrl):
    ...
    for mac in MACs:
        html = html.replace('<td>' + mac + '</td>', '<td><a href="' +
                            deviceUrl + mac + '">' + mac + '</a></td>')
...
def send_email(pText, pHTML):
    ...
    if LOG_LEVEL == 'debug':
        print_log('REPORT_TO: ' + hide_email(str(REPORT_TO)) +
                  '  SMTP_USER: ' + hide_email(str(SMTP_USER)))
    ...
    try:
        # Send mail
        failedAt = print_log('Trying to open connection to ' +
                             str(SMTP_SERVER) + ':' + str(SMTP_PORT))

        if SMTP_FORCE_SSL:
            failedAt = print_log('SMTP_FORCE_SSL == True so using .SMTP_SSL()')
            if SMTP_PORT == 0:
                failedAt = print_log(
                    'SMTP_PORT == 0 so sending .SMTP_SSL(SMTP_SERVER)')
                smtp_connection = smtplib.SMTP_SSL(SMTP_SERVER)
            else:
                failedAt = print_log(
                    'SMTP_PORT == 0 so sending .SMTP_SSL(SMTP_SERVER, SMTP_PORT)')
            ...
            if SMTP_PORT == 0:
                failedAt = print_log(
                    'SMTP_PORT == 0 so sending .SMTP(SMTP_SERVER)')
                smtp_connection = smtplib.SMTP(SMTP_SERVER)
            else:
                failedAt = print_log(
                    'SMTP_PORT == 0 so sending .SMTP(SMTP_SERVER, SMTP_PORT)')
        ...
        mylog('none', [
              '      ERROR: Couldn\'t connect to the SMTP server (SMTPAuthenticationError), skipping Email (enable LOG_LEVEL=debug for more logging)'])
    except smtplib.SMTPServerDisconnected as e:
        mylog('none', ['      ERROR: Failed at - ', failedAt])
        mylog('none', [
              '      ERROR: Couldn\'t connect to the SMTP server (SMTPServerDisconnected), skipping Email (enable LOG_LEVEL=debug for more logging)'])
...
def send_ntfy(_Text):
    ...
    if NTFY_USER != "" and NTFY_PASSWORD != "":
        ...
        basichash = b64encode(
            bytes(NTFY_USER + ':' + NTFY_PASSWORD, "utf-8")).decode("ascii")
...
def send_webhook(_json, _html):
    ...
    if (WEBHOOK_URL.startswith('https://discord.com/api/webhooks/') and not WEBHOOK_URL.endswith("/slack")):
        ...
        curlParams = ["curl", "-i", "-H", "Content-Type:application/json",
                      "-d", json.dumps(_json_payload), _WEBHOOK_URL]
    else:
        _WEBHOOK_URL = WEBHOOK_URL
        curlParams = ["curl", "-i", "-X", WEBHOOK_REQUEST_METHOD, "-H",
                      "Content-Type:application/json", "-d", json.dumps(_json_payload), _WEBHOOK_URL]
    ...
    try:
        ...
        p = subprocess.Popen(
            curlParams, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def send_apprise(html, text):
    ...
    try:
        # try runnning a subprocess
        p = subprocess.Popen(["curl", "-i", "-X", "POST", "-H", "Content-Type:application/json", "-d",
                             json.dumps(_json_payload), APPRISE_HOST], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        stdout, stderr = p.communicate()
        # write stdout and stderr into .log files for debugging if needed
        logResult(stdout, stderr)
    except subprocess.CalledProcessError as e:
...
def create_generic_device(client):
    ...
    create_sensor(client, deviceId, deviceName,
                  'sensor', 'online', 'wifi-check')
    create_sensor(client, deviceId, deviceName,
                  'sensor', 'down', 'wifi-cancel')
    create_sensor(client, deviceId, deviceName, 'sensor', 'all', 'wifi')
    create_sensor(client, deviceId, deviceName,
                  'sensor', 'archived', 'wifi-lock')
    create_sensor(client, deviceId, deviceName, 'sensor', 'new', 'wifi-plus')
    create_sensor(client, deviceId, deviceName,
                  'sensor', 'unknown', 'wifi-alert')


# -------------------------------------------------------------------------------
def create_sensor(client, deviceId, deviceName, sensorType, sensorName, icon):

    new_sensor_config = sensor_config(
        deviceId, deviceName, sensorType, sensorName, icon)
...
class sensor_config:
    def __init__(self, deviceId, deviceName, sensorType, sensorName, icon):
        ...
        self.icon = icon
        self.hash = str(hash(str(deviceId) + str(deviceName) +
                        str(sensorType) + str(sensorName) + str(icon)))
...
def publish_sensor(client, sensorConf):
    ...
    topic = 'homeassistant/'+sensorConf.sensorType+'/' + \
        sensorConf.deviceId+'/'+sensorConf.sensorName+'/config'
...
def mqtt_start():
    ...
    mylog('info', ["        Estimated delay: ", (sec_delay),
          's ', '(', round(sec_delay/60, 1), 'min)'])

    for device in devices:

        # Create devices in Home Assistant - send config messages
        deviceId = 'mac_' + \
            device["dev_MAC"].replace(" ", "").replace(":", "_").lower()
        deviceNameDisplay = re.sub('[^a-zA-Z0-9-_\s]', '', device["dev_Name"])

        create_sensor(client, deviceId, deviceNameDisplay,
                      'sensor', 'last_ip', 'ip-network')
        create_sensor(client, deviceId, deviceNameDisplay,
                      'binary_sensor', 'is_present', 'wifi')
        create_sensor(client, deviceId, deviceNameDisplay,
                      'sensor', 'mac_address', 'folder-key-network')
        create_sensor(client, deviceId, deviceNameDisplay,
                      'sensor', 'is_new', 'bell-alert-outline')
        create_sensor(client, deviceId, deviceNameDisplay,
                      'sensor', 'vendor', 'cog')
...
def upgradeDB():
    ...
    if onlineHistoryAvailable and isIncompatible:
        file_print(
            '[upgradeDB] Table is incompatible, Dropping the Online_History table)')
...
def initOrSetParam(parID, parValue):
    ...
    sql.execute("INSERT INTO Parameters(par_ID, par_Value) VALUES('"+str(parID)+"', '"+str(parValue) +
                "') ON CONFLICT(par_ID) DO UPDATE SET par_Value='"+str(parValue)+"' where par_ID='"+str(parID)+"'")
...
def updateState(newState):
    ...
    sql.execute("UPDATE Parameters SET par_Value='" +
                newState + "' WHERE par_ID='Back_App_State'")
...
def update_api(isNotification=False, updateOnlyDataSources=[]):
    ...
    if isNotification:
        ...
        write_file(folder + 'notification_json_final.json',
                   json.dumps(json_final))
    ...
    for dsSQL in dataSourcesSQLs:
        ...
        if updateOnlyDataSources == [] or dsSQL[0] in updateOnlyDataSources:
            ...
            api_endpoint_class(dsSQL[1], folder +
                               'table_' + dsSQL[0] + '.json')
...
class api_endpoint_class:
    def __init__(self, sql, path):
        ...
        if not found or changed:
            ...
            mylog('verbose', [
                  f'     [API] Updating {self.fileName} file in /front/api'])
...
def to_text(_json):
    ...
    if len(_json['internet']) > 0 and 'internet' in INCLUDED_SECTIONS:
        ...
        for event in _json['internet']:
            payloadData += event[3] + ' on ' + event[2] + \
                '. ' + event[4] + '. New address:' + event[1] + '\n'
    ...
    if len(_json['down_devices']) > 0 and 'down_devices' in INCLUDED_SECTIONS:
        write_file(logPath + '/down_devices_example.log',
                   _json['down_devices'])
    ...
    if len(_json['events']) > 0 and 'events' in INCLUDED_SECTIONS:
        ...
        for event in _json['events']:
            if event[8] != "Internet":
                payloadData += event[8] + " on " + event[1] + \
                    " " + event[3] + " at " + event[2] + "\n"
...
def check_and_run_event():
    ...
    sql.execute(
        "UPDATE Parameters SET par_Value='finished' WHERE par_ID='Front_Event'")
...
def handle_test(testType):
    ...
    sample_json_payload = json.loads(get_file_content(
        pialertPath + '/back/webhook_json_sample.json'))[0]["body"]["attachments"][0]["text"]
    ...
    if testType == 'REPORT_PUSHOVER':
        send_pushover(sample_txt)
...
def get_setting(key):
    ...
    if result is None:
        mylog(
            'info', [' Error - setting_missing - Setting not found for key: ', key])
        mylog('info', [' Error - logging the settings into file: ',
              logPath + '/setting_missing.json'])
        write_file(logPath + '/setting_missing.json',
                   json.dumps({'data': mySettings}))
...
def isNewVersion():
    ...
    if newVersionAvailable == False:
        ...
        try:
            url = requests.get(
                "https://api.github.com/repos/jokob-sk/Pi.Alert/releases")
            text = url.text
        ...
        if data != "" and len(data) > 0 and isinstance(data, list) and "published_at" in data[0]:
            ...
            realeaseTimestamp = int(datetime.datetime.strptime(
                dateTimeStr, '%Y-%m-%dT%H:%M:%SZ').strftime('%s'))

            if realeaseTimestamp > buildTimestamp + 600:
                mylog('none', ["    New version of the container available!"])
                newVersionAvailable = True
                initOrSetParam('Back_New_Version_Available',
...
def get_plugins_configs():
    ...
    for root, dirs, files in os.walk(pluginsPath):
        for d in dirs:            # Loop over directories, not files
            pluginsList.append(json.loads(get_file_content(
                pluginsPath + "/" + d + '/config.json')))
...
def collect_lang_strings(json, pref):
    ...
    for prop in json["localized"]:
        for language_string in json[prop]:
            import_language_string(
                language_string["language_code"], pref + "_" + prop, language_string["string"])


# -------------------------------------------------------------------------------
def import_language_string(code, key, value, extra=""):

    sql.execute("""INSERT INTO Plugins_Language_Strings ("Language_Code", "String_Key", "String_Value", "Extra") VALUES (?, ?, ?, ?)""", (str(
        code), str(key), str(value), str(extra)))
...
def run_plugin_scripts(runType):
    ...
    mylog('debug', [
          '     [Plugins] Check if any plugins need to be executed on run type: ', runType])
                ...
                for schd in mySchedules:
                    if schd.service == prefix:
                        ...
                        if shouldRun:
                            ...
                            schd.last_run = datetime.datetime.now(
                                tz).replace(microsecond=0)

        if shouldRun:

            print_plugin_info(plugin, ['display_name'])
            mylog('debug', ['     [Plugins] CMD: ',
                  get_plugin_setting(plugin, "CMD")["value"]])
...
def execute_plugin(plugin):
    ...
    if "params" in plugin:
        for param in plugin["params"]:
            ...
            if resolved == None:
                mylog('none', ['     [Plugins] The parameter "name":"',
                      param["name"], '" was resolved as None'])
    ...
    if plugin['data_source'] == 'python-script':
        ...
        try:
            ...
            output = subprocess.check_output(
                command, universal_newlines=True,  stderr=subprocess.STDOUT, timeout=(set_RUN_TIMEOUT))
        except subprocess.CalledProcessError as e:
            # An error occured, handle it
            mylog('none', [e.output])
            mylog(
                'none', ['        [Plugins] Error - enable LOG_LEVEL=debug and check logs'])
        except subprocess.TimeoutExpired as timeErr:
            mylog('none', [
                  '        [Plugins] TIMEOUT - the process forcefully terminated as timeout reached'])

        #  check the last run output
        f = open(pluginsPath + '/' +
                 plugin["code_name"] + '/last_result.log', 'r+')
        newLines = f.read().split('\n')
        f.close()
        ...
        for line in newLines:
            ...
            if len(columns) == 9:
                sqlParams.append((plugin["unique_prefix"], columns[0], columns[1], 'null', columns[2],
                                 columns[3], columns[4], columns[5], columns[6], 0, columns[7], 'null', columns[8]))
            else:
                mylog(
                    'none', ['        [Plugins]: Skipped invalid line in the output: ', line])
    ...
    if plugin['data_source'] == 'pialert-db-query':
        ...
        for row in arr:
            ...
            if len(row) == 9 and (row[0] in ['', 'null']) == False:
                sqlParams.append((plugin["unique_prefix"], row[0], row[1], 'null',
                                 row[2], row[3], row[4], row[5], row[6], 0, row[7], 'null', row[8]))
            else:
                mylog(
                    'none', ['        [Plugins]: Skipped invalid sql result'])

    # check if the subprocess / SQL query failed / there was no valid output
    if len(sqlParams) == 0:
        mylog('none', ['        [Plugins] No output received from the plugin ',
              plugin["unique_prefix"], ' - enable LOG_LEVEL=debug and check logs'])
        return
    else:
        mylog('verbose', ['[', timeNow(), '] [Plugins]: SUCCESS, received ', len(
            sqlParams), ' entries'])
...
def process_plugin_events(plugin):
    ...
    plugObjectsArr = get_sql_array(
        "SELECT * FROM Plugins_Objects where Plugin = '" + str(pluginPref)+"'")
    plugEventsArr = get_sql_array(
        "SELECT * FROM Plugins_Events where Plugin = '" + str(pluginPref)+"'")
    ...
    mylog('debug', ['     [Plugins] Existing objects        : ',
          existingPluginObjectsCount])
    mylog(
        'debug', ['     [Plugins] New and existing events : ', len(plugEventsArr)])
    ...
    for plugObj in pluginObjects:
        ...
        if plugObj.status == 'new':
            ...
            sql.execute("INSERT INTO Plugins_Objects (Plugin, Object_PrimaryID, Object_SecondaryID, DateTimeCreated, DateTimeChanged, Watched_Value1, Watched_Value2, Watched_Value3, Watched_Value4, Status, Extra, UserData, ForeignKey) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (plugObj.pluginPref, plugObj.primaryId, plugObj.secondaryId, createdTime, plugObj.changed, plugObj.watched1, plugObj.watched2, plugObj.watched3, plugObj.watched4, plugObj.status, plugObj.extra, plugObj.userData, plugObj.foreignKey))
        else:
    ...
    for plugObj in pluginEvents:
        ...
        if plugObj.status in get_plugin_setting_value(plugin, "REPORT_ON"):
            ...
            sql.execute("INSERT INTO Plugins_Events (Plugin, Object_PrimaryID, Object_SecondaryID, DateTimeCreated, DateTimeChanged, Watched_Value1, Watched_Value2, Watched_Value3, Watched_Value4, Status,  Extra, UserData, ForeignKey) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (plugObj.pluginPref, plugObj.primaryId, plugObj.secondaryId, createdTime, plugObj.changed, plugObj.watched1, plugObj.watched2, plugObj.watched3, plugObj.watched4, plugObj.status, plugObj.extra, plugObj.userData, plugObj.foreignKey))
    ...
    if len(pluginEvents) > 0 and "mapped_to_table" in plugin:
        ...
        mylog(
            'debug', ['     [Plugins] Mapping objects to database table: ', dbTable])
...
class plugin_object_class:
    def __init__(self, plugin, objDbRow):
        ...
        indexNameColumnMapping = [
            (6, 'Watched_Value1'), (7, 'Watched_Value2'), (8, 'Watched_Value3'), (9, 'Watched_Value4')]
    ...
    for param in params:
        ...
        for comPart in commandArr:
            ...
            commandArr[i] = comPart.replace(
                '{' + param[0] + '}', param[1]).replace('{s-quote}', "'")
...
def plugin_param_from_glob_set(globalSetting):
    ...
    noConversion = ['text', 'integer', 'boolean', 'password',
                    'readonly', 'selectinteger', 'selecttext']
...
def get_plugin_setting(plugin, function_key):
    ...
    if result == None:
        mylog('none', ['     [Plugins] Setting with "function":"', function_key,
              '" is missing in plugin: ', get_plugin_string(plugin, 'display_name')])
    ...
    def runScheduleCheck(self):
        ...
        if self.last_run == 0:
            self.last_run = (datetime.datetime.now(
                tz) - timedelta(days=365)).replace(microsecond=0)
