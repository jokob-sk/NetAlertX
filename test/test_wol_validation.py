...
def ensure_backend_ready():
    ...
    if not wait_for_server():
        pytest.skip("NetAlertX backend is not reachable for WOL validation tests", allow_module_level=True)
...
def auth_headers() -> Dict[str, str]:
    ...
    if not token:
        pytest.skip("API_TOKEN not configured; skipping WOL validation tests", allow_module_level=True)
